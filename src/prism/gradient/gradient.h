...
class IGradient
  ...
  public:
    ...
    virtual auto gradAtFace(const mesh::Face& f, field::IScalar& field) -> Vector3d;
    virtual auto gradAtCell(const mesh::Cell& c, field::IScalar& field) -> Vector3d = 0;
    /// TODO: gradAtCellStored should not require the field as an argument. Remove it.
    virtual auto gradAtCellStored(const mesh::Cell& c, const field::IScalar& field)
        -> Vector3d = 0;

  protected:
    virtual auto gradAtBoundaryFace(const mesh::Face& f, field::IScalar& field) -> Vector3d;
};
