...
class Cell {
  public:
    Cell(const std::vector<Face>& faces,
         std::vector<size_t> faces_ids,
         std::vector<size_t> vertices_ids,
         size_t cell_id);
    ...
    auto facesIds() const noexcept -> const std::vector<size_t>& { return _faces_ids; }

    auto facesIds() noexcept -> std::vector<size_t>& { return _faces_ids; }
