...
FacesLookupTrie::FacesLookupTrie(size_t n_vertices) {
...
void FacesLookupTrie::insert(const std::vector<size_t>& face_labels, size_t face_id) {
    ...
    for (size_t i = 1; i < face_labels_size; ++i) {
...
auto FacesLookupTrie::find(const std::vector<size_t>& face_labels) const
    -> std::optional<size_t> {
    auto root_node_id = face_labels[0];
    auto face_labels_size = face_labels.size();
