...
class UnvToPMeshConverter : public ToPMeshConverter {
  ...
  private:
    ...
    using BoundaryFaceData = Pair<size_t, bool>;

    // maps a boundary face Unv element index to BoundaryFaceData defined above
    using UnvIndexToBFaceDataMap = std::map<size_t, BoundaryFaceData>;

    // maps a patch name to a set of faces ids in `_faces` vector
    using PatchNameToFacesIdsMap = std::map<std::string, std::vector<size_t>>;
    ...
    // faces
    auto processFace(std::vector<size_t>& face_vertices) -> size_t;
    auto processBoundaryFace(const unvpp::Element& boundary_face) -> size_t;
    ...
    auto faceIndex(const std::vector<size_t>& face_vertices) const
        -> std::optional<size_t>;
    ...
    std::vector<size_t> _boundary_faces;
    std::vector<size_t> _interior_faces;
    std::vector<BoundaryPatch> _boundary_patches;
    std::vector<FieldInfo> _field_infos;
    size_t _cell_id_counter {0};
    size_t _face_id_counter {0};
};
