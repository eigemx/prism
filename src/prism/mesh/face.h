...
public:
    ...
    auto inline id() const -> const std::size_t& { return _id; }
    auto inline owner() const -> const std::size_t& { return _owner; }
    void inline set_owner(std::size_t owner_id) { _owner = owner_id; }
    auto inline neighbor() const -> const std::optional<std::size_t>& {
        return _neighbor;
    }
    auto inline set_neighbor(std::size_t nei_id) { _neighbor = nei_id; }

private:
    void set_face_attributes(const std::vector<Vector3d>& face_vertices,
